function calendar([day,month,year]) {
    ...
    $('#content').append($('<table>').append($('<caption>').text(monthName)).append($('<tbody>')))
    $('tbody').append($('<tr>').append($('<th>').text("Mon")).append($('<th>').text("Tue"))
        .append($('<th>').text("Wed")).append($('<th>').text("Thu")).append($('<th>')
        .text("Fri")).append($('<th>').text("Sat")).append($('<th>').text("Sun")))
    let numberOfDays = 0
